...
#init globals 1
carat_to_grp = pd.read_excel("rusprice/CARtoKVG_v2.xlsx", sheet_name= 3).iloc[:,0:3]
...
PRD = pd.read_excel('rusprice/PR2019_270921_to_PR2022.xlsx').set_index('Код')
PRD.columns = ['OGR', 'GRUP', 'KVG', 'PO', 'Col', 'Cla', 'Pst', 'Carat', 'Pr2018',
       'Pr2019', 'Pr2022old', 'Pr2022']
PRD[['Pr2018','Pr2019','Pr2022old','Pr2022']] = PRD[['Pr2018','Pr2019','Pr2022old','Pr2022']].astype('float64')
...
def rpdiamonds(df_original):
    ...
    res0 = pd.merge(diamonds,PRD[['ogrbasic','Grp','Col','Cla','Pr2022']], how='left',left_on=['ogrbasic','Grp','Col','Cla'], right_on=['ogrbasic','Grp','Col','Cla'])
    res=pd.merge(res0,ogrdiscount[['ogrdisc','Grp','ogrdiscount']], how = 'left', left_on = ['ogrdisc','Grp'], right_on =  ['ogrdisc','Grp'])
    res['prpr'] = round(res.Pr2022*res.ogrdiscount*res.discont_po+0.0000001,0)
    ...
    print('не сработал прейскурант на этих строках:\n  ', res.loc[pd.isna(res.prpr),['ШТУК',
       ...
       'Cla', 'Po', 'ogrdisc', 'Grp', 'discont_po', 'ogrbasic', 'Pr2022',
